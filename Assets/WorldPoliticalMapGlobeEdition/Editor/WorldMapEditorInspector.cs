...
namespace WPM {
    ...
    public class WorldMapEditorInspector : Editor {
        ...
        bool ShowRegionsGroup(IAdminEntity entity, int currentSelectedRegionIndex) {
            ...
            Region region = entity.regions[currentSelectedRegionIndex];
            if (region.latlon != null) {
                sb.Length = 0;
                for (int k = 0; k < region.latlon.Length; k++) {
                    sb.Append(region.latlon[k].x);
                    sb.Append(", ");
                    sb.Append(region.latlon[k].y);
                    sb.Append("  ");
                }
                EditorGUILayout.LabelField("Selected region points (lat/lon):");
            ...
            switch (_editor.operationMode) {
                case OPERATION_MODE.SELECTION:
                    ...
                    if (mouseDown) {
                        ...
                        if (!_editor.CitySelectByScreenClick(ray, _editor.countryIndex)) {
                            _editor.ClearCitySelection();
                        }
                        if (!_editor.MountPointSelectByScreenClick(ray, _editor.countryIndex)) {
        ...
        void ExecuteMoveTool() {
            ...
            if (_editor.reshapeRegionMode == RESHAPE_REGION_TOOL.POINT && startedReshapeRegion) {
                ...
                if (Camera.current != null) {
                    ...
                    DrawCursorLatLonLabel(Camera.current);
                    string text = "Hotkeys: Shift+S to snap to nearest vertex";
            ...
            if (!frontiersUnchanged) {
                ...
                if (affectedRegions.Count > 0) {
                    _editor.RedrawFrontiers(affectedRegions, false, forceDrawFrontiers);
                    HandleUtility.Repaint();
                }
            }
        }
        ...
        bool SaveChanges() {
            ...
            if (_editor.mountPointChanges) {
                ...
                string data = _editor.GetMountPointGeoData();
                if (data != null) {
                    File.WriteAllText(fullPathName, data, Encoding.UTF8);
            ...
            if (_map.GetGlobeIntersection(ray, out cursorPos)) {
                ...
                if (numPoints > 2) { // Check if we're over the first point
                    ...
                    if (i == 0) {
                        ...
                        if (Camera.current != null) {
                            ...
                            labelPos = Camera.current.ScreenToWorldPoint(screenPos + Misc.Vector3up * 30f + Misc.Vector3right * 30f);
                        } else {
                ...
                if (mouseDown) {
                    if (canClosePolygon) {
                        ...
                        _editor.AddPointToShape(_editor.cursor);
                    }
                }
