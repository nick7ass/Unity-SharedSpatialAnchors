	...
	public static class Drawing {
		...
		public static GameObject CreateSurface (string name, Vector3[] surfPoints, int maxIndex, Material material, Rect rect, Vector2 textureScale, Vector2 textureOffset, float textureRotation) {
			...
			for (int k = 0; k <= maxIndex; k++) {
				...
				int tn;
				if (hit.TryGetValue (p, out tn)) {
					triNew [k] = tn;
				} else {
					newPoints.Add (p);
					hit [p] = ++newPointsIndex;
					triNew [k] = newPointsIndex;
				}
			}
			Mesh mesh = new Mesh ();
			...
			if (material.HasProperty ("_MainTex") && material.mainTexture != null) {
				Vector2[] uv = new Vector2[newPointsIndex + 1];
				for (int k = 0; k < uv.Length; k++) {
					Vector2 coor = Conversion.GetBillboardPosFromSpherePoint (newPoints [k]);
		...
		public static TextMesh CreateText (string text, GameObject parent, int layer, Vector2 center, Font labelFont, Color textColor, bool showShadow, Material shadowMaterial, Color shadowColor, TextAnchor textAnchor = TextAnchor.MiddleCenter, float shadowOffset = 1f) {
			// create base text
			...
			// add shadow
			if (showShadow) {
				GameObject shadow = Object.Instantiate (textObj);
				shadow.name = "shadow";
				shadow.transform.SetParent (textObj.transform, false);
				shadow.transform.localScale = Misc.Vector3one;
				shadow.transform.localPosition = new Vector3 (Mathf.Max (center.x / 100.0f, 1) * shadowOffset, Mathf.Min (center.y / -100.0f, -1) * shadowOffset, 0);
		...
		public static TextMeshPro CreateTextPro(string text, GameObject parent, int layer, Vector2 center, TMP_FontAsset labelFont, Material fontMaterial, bool instantiateMaterial, Color textColor, bool addShadow, float shadowOffset, Color shadowColor) {
			...
			if (instantiateMaterial) {
				fontMaterial = Object.Instantiate(fontMaterial);
			}
			TextMeshPro tmPro = textObj.AddComponent<TextMeshPro>();
			tmPro.font = labelFont;
		...
		public static void ReverseSphereNormals (GameObject gameObject, bool inverted, bool useHighDensityMesh) {
			...
			if (mf != null) {
				...
				if (mesh == null || (currentMeshIsHighDensity && !useHighDensityMesh)) {
					mesh = Object.Instantiate (Resources.Load<Mesh> ("Meshes/SphereMedTris"));
					mf.sharedMesh = mesh;
					gameObject.transform.localRotation = Misc.QuaternionZero; // Quaternion.Euler (0f, 0, 0);
					reloadedMesh = true;
				} else if (!currentMeshIsHighDensity && useHighDensityMesh) {
					mesh = Object.Instantiate (Resources.Load<Mesh> ("Meshes/SphereHighTris"));
					mf.sharedMesh = mesh;
		...
		public static GameObject DrawCircle (string name, Vector3 localPosition, float width, float height, float angleStart, float angleEnd, float ringWidthMin, float ringWidthMax, int numSteps, Material material, bool correctDistortion) {
			...
			MeshFilter meshFilter = hexa.GetComponent<MeshFilter> ();
			meshFilter.mesh = mesh;
			Renderer renderer = hexa.GetComponent<Renderer> ();
		...
		public static void UpdateLineMeshFast (Mesh mesh, Vector3[] currentMeshVertices, Vector3[] referenceMeshVertices, float progress) {
			if (mesh == null || referenceMeshVertices == null || currentMeshVertices == null)
				return;
